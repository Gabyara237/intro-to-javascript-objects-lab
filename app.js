...
game.party.push(pokemon.find(pokemonStarter => pokemonStarter.starter === true));
...
const pokemonToEvolve = game.party.find(currentPokemon => currentPokemon.starter === true);
const pokemonToEvolveIndex = game.party.findIndex(currentPokemon => currentPokemon.name === pokemonToEvolve.name )
const index =  pokemon.findIndex(currentPokemon => currentPokemon.number === pokemonToEvolve.number);
game.party.splice(pokemonToEvolveIndex,1,pokemon[index+1]);
console.log(game.party);
...
pokemon.forEach((currentPokemon)=>{
    if(currentPokemon.starter === true){
...
const comparation = (pokemonA,pokemonB) => {
    return pokemonB.hp-pokemonA.hp;
}
game.party.sort(comparation);
...
const catchPokemon = (pokemonObj) => {
    ...
    const pokeballsInfo = (game.items.find( item => item.name === "pokeball"));

    if (pokeballsInfo.quantity > 0){
        if (game.party.length < 6){
            game.party.push(pokemonObj);
        }else{
            game.collection.push(pokemonObj);
        ...
        pokeballsInfo.quantity-- ; 
    
    }else{
        console.log("Sorry, there are not enough pokeballs to catch the desired Pokemon.")
    }
}
